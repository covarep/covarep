...
classdef HammingWindow < WindowFunction

    methods (Static)
        function window = get_impulse_response(obj, width)
            % Impulse response of a unit-normalized Hamming window
            % 
            % Inputs
            ...
            % Example
            % >> width = 64;
            % >> window = HammingWindow.get_impulse_response(width);
            % 
            
            window = hamming(width);
            window = window / (0.54 * max(1, width - 1));
        end
    end
end