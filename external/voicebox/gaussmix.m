...
%      Version: $Id: gaussmix.m 3119 2013-06-19 17:47:55Z dmb $
%
...
if nargin<5 || isempty(v0) || ischar(v0)             % no initial values specified for m0, v0, w0
    ...
    if nargin<6
        ...
        wt=wn;                      % no data point weights
    else
        wt=w0(:);                   % data point weights
    end
    if any(v0=='m')
        k=size(m0,1);
    else
...
if length(wt)~=n
    error('%d datapoints but %d weights',n,length(wt));
end
lsx=sum(log(sx0));
xsw=xs.*repmat(wt,1,p); % weighted data points
nwt=sum(wt);        % number of data points counting duplicates
if ~fulliv          % initializing with diagonal covariance
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Diagonal Covariance matrices  %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    v=max(v,c);         % apply the lower bound
    xs2=xs.^2.*repmat(wt,1,p);          % square and weight the data for variance calculations
    ...
    for j=1:lp
        ...
        lpx(ii)=log(ps)+mx;
        pk=px*wt(ii);                       % pk(k,1) effective number of data points for each mixture (could be zero due to underflow)
        sx=px*xsw(ii,:);
        sx2=px*xs2(ii,:);
        for il=2:nl                     % process the data points in chunks
            ...
            px=px./ps(wk,:);            % relative mixture probabilities for each data point (columns sum to 1)
            lpx(ii)=log(ps)+mx;
            pk=pk+px*wt(ii);                % pk(k,1) effective number of data points for each mixture (could be zero due to underflow)
            sx=sx+px*xsw(ii,:);
            sx2=sx2+px*xs2(ii,:);
        end
        g=lpx*wt;                       % total log probability summed over all data points
...
if fv              % check if full covariance matrices were requested
    ...
    for j=1:lp
        ...
        px=px./ps(wk,:);                % relative mixture probabilities for each data point (columns sum to 1)
        lpx(ii)=log(ps)+mx;
        pk=px*wt(ii);                       % effective number of data points for each mixture (could be zero due to underflow)
        sx=px*xsw(ii,:);
        ...
        for il=2:nl
            ...
            px=px./ps(wk,:);                % relative mixture probabilities for each data point (columns sum to 1)
            lpx(ii)=log(ps)+mx;
            pk=pk+px*wt(ii);                    % effective number of data points for each mixture (could be zero due to underflow)
